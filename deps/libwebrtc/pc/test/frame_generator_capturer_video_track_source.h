 ...
 * @LastEditTime: 2024-10-30 01:06:17
 * @FilePath:
 * /mediasoup-broadcaster-demo/deps/libwebrtc/pc/test/frame_generator_capturer_video_track_source.h
...
public:
	...
	FrameGeneratorCapturerVideoTrackSource(Config config, Clock* clock, bool is_screencast)
	  : VideoTrackSource(false /* remote */), task_queue_factory_(CreateDefaultTaskQueueFactory()),
	    is_screencast_(is_screencast)
	{
		video_capturer_ = std::make_unique<test::FrameGeneratorCapturer>(
		  clock,
		  test::CreateSquareFrameGenerator(
		    config.width, config.height, absl::nullopt, config.num_squares_generated),
	...
	FrameGeneratorCapturerVideoTrackSource(
	  ...
	  : VideoTrackSource(false /* remote */), video_capturer_(std::move(video_capturer)),
	    is_screencast_(is_screencast)
	{
	}
