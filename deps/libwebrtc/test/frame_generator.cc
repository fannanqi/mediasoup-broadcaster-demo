...
namespace test
{
	...
	rtc::scoped_refptr<I420Buffer> SquareGenerator::CreateI420Buffer(int width, int height)
	{
	...
	FrameGeneratorInterface::VideoFrameData SquareGenerator::NextFrame()
	{
		...
		switch (type_)
		{
			...
			case OutputType::kI420A: {
				rtc::scoped_refptr<I420Buffer> yuv_buffer = CreateI420Buffer(width_, height_);
				rtc::scoped_refptr<I420Buffer> axx_buffer = CreateI420Buffer(width_, height_);
	...
	SquareGenerator::Square::Square(int width, int height, int seed)
	  : random_generator_(seed), x_(random_generator_.Rand(0, width)),
	    y_(random_generator_.Rand(0, height)),
	    length_(random_generator_.Rand(1, width > 4 ? width / 4 : 1)),
	    yuv_y_(random_generator_.Rand(0, 255)), yuv_u_(random_generator_.Rand(0, 255)),
	    yuv_v_(random_generator_.Rand(0, 255)), yuv_a_(random_generator_.Rand(0, 255))
	{
	}

	void SquareGenerator::Square::Draw(const rtc::scoped_refptr<VideoFrameBuffer>& frame_buffer)
	{
		RTC_DCHECK(
	...
	YuvFileGenerator::YuvFileGenerator(
	  std::vector<FILE*> files, size_t width, size_t height, int frame_repeat_count)
	  : file_index_(0), frame_index_(std::numeric_limits<size_t>::max()), files_(files), width_(width),
	    height_(height), frame_size_(CalcBufferSize(
	                       VideoType::kI420, static_cast<int>(width_), static_cast<int>(height_))),
	    frame_buffer_(new uint8_t[frame_size_]), frame_display_count_(frame_repeat_count),
	...
	FrameGeneratorInterface::VideoFrameData YuvFileGenerator::NextFrame()
	{
		...
		if (current_display_count_ == 0)
		{
			...
			{
				update_rect =
				  VideoFrame::UpdateRect{ 0, 0, static_cast<int>(width_), static_cast<int>(height_) };
			}
		}
	...
	bool YuvFileGenerator::ReadNextFrame()
	{
		...
		last_read_buffer_ =
		  test::ReadI420Buffer(static_cast<int>(width_), static_cast<int>(height_), files_[file_index_]);
		++frame_index_;
	...
	NV12FileGenerator::NV12FileGenerator(
	  ...
	  : file_index_(0), frame_index_(std::numeric_limits<size_t>::max()), files_(files), width_(width),
	    height_(height), frame_size_(CalcBufferSize(
	                       VideoType::kNV12, static_cast<int>(width_), static_cast<int>(height_))),
	    frame_buffer_(new uint8_t[frame_size_]), frame_display_count_(frame_repeat_count),
	    current_display_count_(0)
	{
		RTC_DCHECK_GT(width, 0);
	...
	bool NV12FileGenerator::ReadNextFrame()
	{
		...
		size_t prev_file_index  = file_index_;
		last_read_buffer_ =
		  test::ReadNV12Buffer(static_cast<int>(width_), static_cast<int>(height_), files_[file_index_]);
		++frame_index_;
		if (!last_read_buffer_)
		{
			// No more frames to read in this file, rewind and move to next file.
	...
	SlideGenerator::SlideGenerator(int width, int height, int frame_repeat_count)
	  : width_(width), height_(height), frame_display_count_(frame_repeat_count),
	    current_display_count_(0), random_generator_(1234)
	{
		RTC_DCHECK_GT(width, 0);
		RTC_DCHECK_GT(height, 0);
		RTC_DCHECK_GT(frame_repeat_count, 0);
	...
	ScrollingImageFrameGenerator::ScrollingImageFrameGenerator(
	  ...
	  : clock_(clock), start_time_(clock->TimeInMilliseconds()), scroll_time_(scroll_time_ms),
	    pause_time_(pause_time_ms), num_frames_(files.size()),
	    target_width_(static_cast<int>(target_width)),
	    target_height_(static_cast<int>(target_height)), current_frame_num_(num_frames_ - 1),
	    prev_frame_not_scrolled_(false), current_source_frame_(nullptr, absl::nullopt),
	    current_frame_(nullptr, absl::nullopt), file_generator_(files, source_width, source_height, 1)
	{
	...
	void ScrollingImageFrameGenerator::CropSourceToScrolledImage(double scroll_factor)
	{
		...
		rtc::scoped_refptr<I420BufferInterface> i420_buffer = current_source_frame_.buffer->ToI420();
