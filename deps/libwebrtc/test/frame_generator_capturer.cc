...
namespace test
{
	...
	FrameGeneratorCapturer::FrameGeneratorCapturer(
	  ...
	  : clock_(clock), sending_(true), sink_wants_observer_(nullptr),
	    frame_generator_(std::move(frame_generator)), source_fps_(target_fps),
	    target_capture_fps_(target_fps), first_frame_capture_time_(-1),
	...
	void FrameGeneratorCapturer::SetFakeColorSpace(absl::optional<ColorSpace> color_space)
	{
		MutexLock lock(&lock_);
	...
	absl::optional<FrameGeneratorCapturer::Resolution> FrameGeneratorCapturer::GetResolution() const
	{
	...
	void FrameGeneratorCapturer::ChangeFramerate(int target_framerate)
	{
		...
		if (target_framerate > source_fps_)
			RTC_LOG(LS_WARNING) << "Target framerate clamped from " << target_framerate << " to "
			                    << source_fps_;
		if (source_fps_ % target_capture_fps_ != 0)
		{
			...
			RTC_LOG(LS_WARNING) << "Target framerate, " << target_framerate
			                    << ", is an uneven fraction of the source rate, " << source_fps_
	...
	void FrameGeneratorCapturer::AddOrUpdateSink(
	  rtc::VideoSinkInterface<VideoFrame>* sink, const rtc::VideoSinkWants& wants)
	{
	...
	void FrameGeneratorCapturer::RemoveSink(rtc::VideoSinkInterface<VideoFrame>* sink)
	{
