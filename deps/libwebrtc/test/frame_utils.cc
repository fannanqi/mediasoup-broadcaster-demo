...
namespace test
{
	...
	bool EqualPlane(
	  const uint8_t* data1, const uint8_t* data2, int stride1, int stride2, int width, int height)
	{
	...
	{
		if (
		  f1.timestamp() != f2.timestamp() || f1.ntp_time_ms() != f2.ntp_time_ms() ||
	...
	bool FrameBufsEqual(
	  const rtc::scoped_refptr<webrtc::VideoFrameBuffer>& f1,
	  const rtc::scoped_refptr<webrtc::VideoFrameBuffer>& f2)
	{
		...
		if (f1->width() != f2->width() || f1->height() != f2->height() || f1->type() != f2->type())
		{
			return false;
		}
