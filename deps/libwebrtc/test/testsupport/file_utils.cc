...
namespace test
{
	...
	std::string TempFilename(absl::string_view dir, absl::string_view prefix)
	{
		...
		if (::GetTempFileNameW(rtc::ToUtf16(dir).c_str(), rtc::ToUtf16(prefix).c_str(), 0, filename) != 0)
	...
	std::string GenerateTempFilename(absl::string_view dir, absl::string_view prefix)
	{
